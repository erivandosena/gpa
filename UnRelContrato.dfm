object FrmRelContrato: TFrmRelContrato
  Left = 566
  Top = 296
  Width = 804
  Height = 524
  Caption = 'Pr'#233'-visualiza'#231#227'o de Impress'#227'o'
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Times New Roman'
  Font.Style = []
  OldCreateOrder = False
  Scaled = False
  WindowState = wsMaximized
  PixelsPerInch = 96
  TextHeight = 15
  object QuickRep1: TQuickRep
    Left = 6
    Top = 8
    Width = 794
    Height = 1123
    Frame.Color = clBlack
    Frame.DrawTop = False
    Frame.DrawBottom = False
    Frame.DrawLeft = False
    Frame.DrawRight = False
    DataSet = FrmContrato.IBQuery_Cliente
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    Functions.Strings = (
      'PAGENUMBER'
      'COLUMNNUMBER'
      'REPORTTITLE'
      'QRSTRINGSBAND1')
    Functions.DATA = (
      '0'
      '0'
      #39#39
      #39#39)
    Options = [FirstPageHeader, LastPageFooter]
    Page.Columns = 1
    Page.Orientation = poPortrait
    Page.PaperSize = A4
    Page.Values = (
      50.000000000000000000
      2970.000000000000000000
      50.000000000000000000
      2100.000000000000000000
      100.000000000000000000
      100.000000000000000000
      0.000000000000000000)
    PrinterSettings.Copies = 1
    PrinterSettings.Duplex = False
    PrinterSettings.FirstPage = 0
    PrinterSettings.LastPage = 0
    PrinterSettings.OutputBin = Auto
    PrintIfEmpty = True
    SnapToGrid = True
    Units = MM
    Zoom = 100
    object DetailBand1: TQRBand
      Left = 38
      Top = 19
      Width = 718
      Height = 678
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        1793.875000000000000000
        1899.708333333333000000)
      BandType = rbDetail
      object QRExprMemo1: TQRExprMemo
        Left = 1
        Top = 120
        Width = 717
        Height = 585
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          1547.812500000000000000
          2.645833333333333000
          317.500000000000000000
          1897.062500000000000000)
        RemoveBlankLines = False
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = False
        AutoStretch = False
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          
            'Contrato para presta'#231#227'o de servi'#231'o que fazem de um lado a empres' +
            'a, Carlos Henrique de Holanda Furtado, sediada '#225' rua S'#227'o Paulo, ' +
            'N'#186' 1030 no Centro de Baturit'#233' - Ce, inscrita no CNPJ N'#186' 11.704.2' +
            '02/0001-30, daqui p'#244'r diante designada PLAMMOR (Plano de Assist'#234 +
            'ncia Monte Mor) de outro lado o(a) Sr(a). {NOME_CLIENTE} com dom' +
            'ic'#237'lio na {ENDERECO} na cidade de {CIDADE} - {UF}, sob o documen' +
            'to de Identidade N'#186' {RG}, daqui  p'#244'r diante denominado de CONTRA' +
            'TANTE.'
          ''
          'CL'#193'USULA I - DIREITOS DO CONTRATANTE'
          ''
          
            '1.1 - PLANO A - Sepultamento em urna cl'#225'ssica em madeira, sextav' +
            'ada, ondulada, laqueada, acabamento  em pu, forro, babado e sobr' +
            'ebabado, var'#227'o, c'#226'mara ardente com velas, mesa de c'#244'ndolencias, ' +
            'vestimenta, coroa, ornamenta'#231#227'o com edredom em cetim e flores na' +
            'turais, transporte funeral, transporte para acompanhamento da fa' +
            'm'#237'lia, (6 pessoas) somente dentro do Maci'#231'o de Baturit'#233', certid'#227 +
            'o de '#243'bito, auto-f'#250'nebre  at'#233' 200 Km rodados, servi'#231'o de copa.'
          
            '1.2 - PLANO B - Sepultamento em urna executiva em madeira, sexta' +
            'vada, ondulada, laqueada, acabamento em pu, forro, babado e sobr' +
            'ebabado, tampa inteiri'#231'a com visor, var'#227'o, c'#226'mara ardente com ve' +
            'las, mesa de c'#244'ndolencias, vestimenta, coroa, ornamenta'#231#227'o com e' +
            'dredom, e flores naturais, transporte funeral, transporte para a' +
            'companhamento da fam'#237'lia (10 pessoas), somente  na  regi'#227'o do Ma' +
            'ci'#231'o de Baturit'#233', certid'#227'o de '#243'bito, auto-f'#250'nebre at'#233' 200 Km rod' +
            'ados, servi'#231'o de copa.'
          
            '1.3 - PLANO C - Sepultamento em urna de luxo em madeira, sextava' +
            'da, ondulada, trabalhada, laqueada, acabamento em pu, forro, bab' +
            'ado e sobrebabado em renda, argol'#245'es em dourado, tampa lisa ou c' +
            'om entalhe e visor, cruz e cristo em metal, ou b'#237'blia, c'#226'mara ar' +
            'dente com velas, mesa de condol'#234'ncias, vestimenta em cetim, orna' +
            'menta'#231#227'o com edredom em cetim e flores naturais, coroa, transpor' +
            'te funeral, transporte para acompanhamento da fam'#237'lia (14/46 pes' +
            'soas) somente na regi'#227'o do Maci'#231'o de Baturit'#233', certid'#227'o de '#243'bito' +
            ', auto-f'#250'nebre at'#233' 200 Km rodados, servi'#231'o de copa.'
          ''
          'CL'#193'USULA II - OBRIGA'#199#213'ES'
          ''
          
            '2.1 - Pagamento '#233' mensal, n'#227'o podendo atrasar sob pena de serem ' +
            'cancelados os direitos do CONTRATANTE.'
          
            '2.2 - A CONTRATANTE e seus dependentes s'#243' ter'#227'o os direitos acim' +
            'a ap'#243's 90 dias contando da data deste contrato, exceto nos casos' +
            ' de transfer'#234'ncia de outros planos.'
          
            '2.3 - P'#244'r ocasi'#227'o do falecimento do CONTRATANTE ou de um de seus' +
            ' dependentes, deve-se comunicar imediatamente a PLAMMOR, sem tom' +
            'ar medida alguma de ordem financeira. Caso isto aconte'#231'a a PLAMM' +
            'OR n'#227'o assumir'#225' nenhuma responsabilidade, ficando desde j'#225'  isen' +
            'ta  ao ressarcimento dessas despesas.                      '
          
            '2.4 - Atraso de 3 (tr'#234's) mensalidades seguidas implicar'#225' na perd' +
            'a de 50% dos direitos do CONTRATANTE p'#244'r um per'#237'odo de 90 dias a' +
            'p'#243's o pagamento, independente de qualquer  justificativa. Suspen' +
            'de os direitos do CONTRATANTE e seus dependentes em casos de epi' +
            'demias, cat'#225'strofes, revolu'#231#245'es, guerras  civil  ou qualquer  ou' +
            'tro motivo de for'#231'a maior. Nesses casos cobramos 50% do valor da' +
            ' tabela oficial da associa'#231#227'o das Funer'#225'rias.'
          
            '2.5 - Os direitos do CONTRATANTE estendem-se somente aos depende' +
            'ntes relacionados neste contrato.'
          
            '2.6 - Fica eleito o foro da Comarca de Baturit'#233' para dirimir qua' +
            'isquer d'#250'vidas decorrentes da execu'#231#227'o deste instrumento.'
          ''
          
            'E por estarem assim justas e contratadas, as partes assinam o pr' +
            'esente contrato em vias de igual teor.'
          ''
          'Titular: {NOME_CLIENTE} C'#243'digo: {CODIGO}')
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
      object QRImageLogo: TQRImage
        Left = 64
        Top = 2
        Width = 227
        Height = 65
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          171.979166666666700000
          169.333333333333300000
          5.291666666666667000
          600.604166666666700000)
        AutoSize = True
        Center = True
        Picture.Data = {
          
          C12002003B}
      end
      object QRLabel6: TQRLabel
        Left = 397
        Top = 0
        Width = 146
        Height = 32
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          84.666666666666670000
          1050.395833333333000000
          0.000000000000000000
          386.291666666666700000)
        Alignment = taCenter
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'PLAMMOR'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -27
        Font.Name = 'Times New Roman'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 20
      end
      object QRLabel4: TQRLabel
        Left = 433
        Top = 26
        Width = 73
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          1145.645833333333000000
          68.791666666666670000
          193.145833333333300000)
        Alignment = taCenter
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'A SUA AMIGA'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Times New Roman'
        Font.Style = []
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
      object QRMemo2: TQRMemo
        Left = 294
        Top = 40
        Width = 352
        Height = 41
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          108.479166666666700000
          777.875000000000000000
          105.833333333333300000
          931.333333333333300000)
        Alignment = taCenter
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          'Rua S'#227'o Paulo, 1030 Centro CEP:62760-000 Baturit'#233' - Ce'
          
            'CNPJ:11.704.202/0001-30 Tel.:(85) 347.1142 E-mail:plammor@bol.co' +
            'm.br')
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
      object QRLabel2: TQRLabel
        Left = 195
        Top = 88
        Width = 328
        Height = 23
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          60.854166666666670000
          515.937500000000000000
          232.833333333333300000
          867.833333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'CONTRATO DE PRESTA'#199#195'O DE SERVI'#199'O'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 12
      end
    end
    object QRSubDetail1: TQRSubDetail
      Left = 38
      Top = 714
      Width = 718
      Height = 12
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        31.750000000000000000
        1899.708333333333000000)
      Master = QuickRep1
      DataSet = IBQuery_Dependentes
      FooterBand = GroupFooterBand1
      PrintBefore = False
      PrintIfEmpty = True
      object QRDBText8: TQRDBText
        Left = 1
        Top = 0
        Width = 106
        Height = 15
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          39.687500000000000000
          2.645833333333333000
          0.000000000000000000
          280.458333333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Color = clWhite
        DataSet = IBQuery_Dependentes
        DataField = 'NOME_DEPENDENTE'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Courier New'
        Font.Style = []
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 8
      end
    end
    object QRGroup1: TQRGroup
      Left = 38
      Top = 697
      Width = 718
      Height = 17
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        44.979166666666670000
        1899.708333333333000000)
      Master = QRSubDetail1
      ReprintOnNewPage = False
      object QRLabel1: TQRLabel
        Left = 1
        Top = 2
        Width = 79
        Height = 17
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          44.979166666666670000
          2.645833333333333000
          5.291666666666667000
          209.020833333333300000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = True
        AutoStretch = False
        Caption = 'Dependentes:'
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        Transparent = False
        WordWrap = True
        FontSize = 9
      end
    end
    object GroupFooterBand1: TQRBand
      Left = 38
      Top = 726
      Width = 718
      Height = 126
      Frame.Color = clBlack
      Frame.DrawTop = False
      Frame.DrawBottom = False
      Frame.DrawLeft = False
      Frame.DrawRight = False
      AlignToBottom = False
      Color = clWhite
      ForceNewColumn = False
      ForceNewPage = False
      Size.Values = (
        333.375000000000000000
        1899.708333333333000000)
      BandType = rbGroupFooter
      object QRExprMemo3: TQRMemo
        Left = 1
        Top = 5
        Width = 717
        Height = 118
        Frame.Color = clBlack
        Frame.DrawTop = False
        Frame.DrawBottom = False
        Frame.DrawLeft = False
        Frame.DrawRight = False
        Size.Values = (
          312.208333333333300000
          2.645833333333333000
          13.229166666666670000
          1897.062500000000000000)
        Alignment = taLeftJustify
        AlignToBand = False
        AutoSize = False
        AutoStretch = False
        Color = clWhite
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Lines.Strings = (
          'Plano: ________'
          '    '
          'Baturit'#233', ______ de _________________________ de 20______.'
          ''
          ''
          
            '                _____________________________________           ' +
            '                     _____________________________________'
          
            '                                     CONTRATANTE                ' +
            '                                                                ' +
            '   PLAMMOR')
        ParentFont = False
        Transparent = True
        WordWrap = True
        FontSize = 9
      end
    end
  end
  object DataSource2: TDataSource
    DataSet = IBQuery_Dependentes
    Left = 16
    Top = 50
  end
  object IBQuery_Dependentes: TIBQuery
    Database = Dm.IBDatabase
    Transaction = Dm.IBTransaction
    Active = True
    DataSource = FrmContrato.Ds_Cliente
    SQL.Strings = (
      
        'select COD_DEPE, CODIGO, NOME_DEPENDENTE, PARENTESCO from DEPEND' +
        'E'
      'where (CODIGO = :CODIGO)'
      'order by NOME_DEPENDENTE')
    Left = 16
    Top = 16
    ParamData = <
      item
        DataType = ftString
        Name = 'CODIGO'
        ParamType = ptUnknown
        Size = 12
      end>
    object IBQuery_DependentesCOD_DEPE: TIntegerField
      FieldName = 'COD_DEPE'
      Origin = '"DEPENDE"."COD_DEPE"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object IBQuery_DependentesCODIGO: TIBStringField
      FieldName = 'CODIGO'
      Origin = '"DEPENDE"."CODIGO"'
      Size = 11
    end
    object IBQuery_DependentesNOME_DEPENDENTE: TIBStringField
      FieldName = 'NOME_DEPENDENTE'
      Origin = '"DEPENDE"."NOME_DEPENDENTE"'
      Size = 50
    end
    object IBQuery_DependentesPARENTESCO: TIBStringField
      FieldName = 'PARENTESCO'
      Origin = '"DEPENDE"."PARENTESCO"'
    end
  end
end
